...
(*  structure R = RegAlloc *)
  ...
  fun emitproc out (F.PROC{body,frame}) =
        ...
        in  
          (app (fn i => (print("!" ^ format0(i));
                        TextIO.output(out,format0 i))) instrs)
        end
    | emitproc out (F.STRING(lab,s)) = 
        (print("!!" ^ s); TextIO.output(out,F.string(lab,s)))

   fun withOpenFile fname f = 
       let 
         val _ = print("[withOpenFile] given " ^ fname ^ "\n")
       ...
       let val absyn = Parse.parse filename
        ...
        in 
          (print("Compiling " ^ filename ^ ".\n");
           print("Size of frags: " ^ Int.toString(length(frags)) ^ "\n");
