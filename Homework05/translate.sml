...
struct
  ...
  * program lives, as well as top-level stdlib functions *)
  ...
  fun unCx(Ex(Tr.CONST(0))) = (fn (t,f) => Tr.JUMP(Tr.NAME(f), [f]))
    ...
    | unCx(Ex e) = 
      ...
      in (fn (t,f) => Tr.SEQ(Tr.MOVE(Tr.MEM(Tr.TEMP r), e),
                           (Tr.CJUMP(Tr.NE, Tr.TEMP r, Tr.CONST 0, t, f))))
      end
    | unCx(Cx genstm) = genstm
    | unCx(Nx s) = ErrorMsg.impossible("unCx'ed an Nx")
    ...
    case elseExpOpt
      of NONE => 
           ...
           * Label ifend at bottom *)
           ...
           in
            Nx(seq[unCx(test)(t, ifend),
                   (* Tr.CJUMP(Tr.EQ, Tr.CONST(1), unEx(test), t, ifend), *)
                   Tr.LABEL(t),
                   unNx(thenExp),
                   Tr.LABEL(ifend)])
           end
  ...
  fun subscriptVar(varexp, offsetexp) =
    Ex(Tr.MEM(Tr.BINOP(Tr.PLUS, unEx(varexp), 
                Tr.BINOP(Tr.MUL, Tr.CONST(Frame.wordSize), unEx(offsetexp)))))
  ...
  fun funDec(label, level : level, body) = 
    procEntryExit({level = level,
                   body = Nx(Tr.SEQ(Tr.LABEL(label), unNx(body)))})
